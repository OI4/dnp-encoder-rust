...
#[test]
fn validate_basic_ok() {
    validate_dnp("ABC123-._~", &Rules::default()).unwrap();
}

#[test]
fn validate_escape_ok() {
    validate_dnp(",2C", &Rules::default()).unwrap();
}

#[test]
fn validate_lone_comma() {
    assert!(validate_dnp(",", &Rules::default()).is_err());
}

#[test]
fn validate_short_escape() {
    assert!(validate_dnp(",2", &Rules::default()).is_err());
}

#[test]
fn validate_invalid_hex_digit() {
    assert!(validate_dnp(",2G", &Rules::default()).is_err());
}
...
fn validate_lowercase_allowed_runtime_rule() {
    let r = Rules {
        enforce_reserved_masking: false,
        allow_lowercase_hex: true,
    };
